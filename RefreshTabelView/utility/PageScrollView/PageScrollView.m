...
//    }
    ...
    for (int i = 0; i < _totalCount; i++) {
        ...
        CustomImageView *imageView = [[CustomImageView alloc] initWithFrame:frame];
        imageView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleLeftMargin;
        imageView.delegate = self;
        [_myScrollView addSubview:imageView];
...
- (void)addPageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleRightMargin;
        _pageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
        _pageControl.frame = CGRectMake((VIEW_WITDTH - 20*imageCount)/2, VIEW_HEIGHT - 40, 20*imageCount, 20);//指定位置大小
        _pageControl.numberOfPages = imageCount;//指定页面个数
        _pageControl.currentPage = 0;//指定pagecontroll的值
        _pageControl.enabled = NO;  //让小圆点不能点
        if (_pageControl.enabled) {
